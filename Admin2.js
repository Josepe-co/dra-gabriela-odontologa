/* ═══════════════════════════════════════════════════════════
   DR. CHRISTIAN CID — ADMIN PANEL JS
   Calendario · Citas · Pacientes · Ingresos · Reportes PDF
   Base de datos independiente (colecciones: citas_christian,
   pacientes_christian, gastos_christian)
═══════════════════════════════════════════════════════════ */
"use strict";

/* ─── NÚMERO DE WHATSAPP DEL DOCTOR ─────────────────────── */
/* ⚠️ Cambia este número por el del Dr. Christian Cid        */
const DOCTOR_WA = "521XXXXXXXXXX";

/* ═══════════════════════════════════════════════════════════
   FIREBASE — Base de datos en la nube (Firestore)
   Usa la segunda app inicializada en Admin2.html ("christian")
═══════════════════════════════════════════════════════════ */
const db           = appChristian.firestore();
const colCitas     = db.collection("citas_christian");
const colPacientes = db.collection("pacientes_christian");
const colGastos    = db.collection("gastos_christian");
...
function enviarWaConfirmacion(cita) {
  ...
  const msg =
    ...
    `Le escribimos del consultorio del *Dr. Christian Cid* para confirmarle que su cita ha sido agendada con éxito.\n\n` +
    `📋 *Detalles de su cita:*\n` +
...
function enviarWaRecordatorio(cita) {
  ...
  const msg =
    ...
    `Le recordamos su cita del día *${formatFechaLarga(cita.fecha)} a las ${formatHoraAmPm(cita.hora)}* en el consultorio del *Dr. Christian Cid*, para su tratamiento de *${cita.motivo}*.\n\n` +
...
function activarRenombre(item, adj, citaId) {
  ...
  if (wrap.querySelector(".adjunto-nombre-input")) return;
...
function renderIngresos(filtro, fechaEspecifica = null) {
  ...
  citasFiltradas.forEach(c => {
    ...
    tr.addEventListener("click", () => abrirDetalleCita(c.id));
...
function generarPDF() {
  ...
  doc.setFontSize(16);
  doc.text("Dr. Christian Cid", 14, 14);
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text("Odontología General · Tuxtla Gutiérrez, Chiapas", 14, 21);
  doc.setFontSize(9);
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...GRAY);
    doc.text(`Dr. Christian Cid · Odontología General · Tuxtla Gutiérrez, Chiapas`, 14, 273);
    doc.text(`Página ${i} de ${pages}`, 202, 273, { align: "right" });
  }

  doc.save(`Reporte_Semana_${semVal}_DrChristian.pdf`);
